...
use DB;
class HourlyGradeController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
            ...
            $monthly_grades = EmpPaygrades::Where("grade_type","Hourly");
            ...
            return view('pay_grade.indexh', compact('monthly_grades'));
        }
        else
        {
    ...
    public function create()
    {
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
            ...
            return view('pay_grade.createh', compact('grade_type','allowanceOption'));
        }
        else
        {
      ...
      //dd($request);
        ...
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
           ...
           // $emppaygrades->allowence        = $request->allowence;
            ...
            return redirect()->route('hourly_grade.index')->with('success', __('Grade  successfully created.'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function edit($id)
    {
        ...
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
                ...
                $allowanceOption=AllowanceOption::all();
                return view('pay_grade.edith', compact('emppaygrades', 'gradeallowance_array','allowanceOption'));
       ...
       // dd($request);
        ...
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
            if($emppaygrades->created_by == Auth::user()->creatorId())
            {
                ...
                return redirect()->route('hourly_grade.index')->with('success', __('Grade successfully updated.'));
            }
            else
            {
                return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function destroy($id)
    {
        ...
        if(\Auth::user()->can('Manage Monthly Grade'))
        {
            if($emppaygrades->created_by == \Auth::user()->creatorId())
            {
              ...
              if(count($gdu)) {
                return redirect()->route('hourly_grade.index')->with('error', __('Grade is being used in app.'));
              } else {
                $emppaygrades->delete();

                EmpGradeAllowance::where("grade_id",$id)->delete();
                return redirect()->route('hourly_grade.index')->with('success', __('Grade successfully deleted.'));
              }
            }
            else
            {
