...
class BranchController extends Controller
{
    ...
    public function destroy(Branch $branch)
    {
        if(\Auth::user()->can('Delete Branch'))
        {
            if($branch->created_by == \Auth::user()->creatorId())
            {
              $emps1 = \App\Employee::where("branch_id",$branch->id)->get();
              $emps = \App\Department::where("branch_id",$branch->id)->get();
              if(count($emps) || count($emps1) ) {
                return redirect()->route('branch.index')->with('error', __('Branch is being used in app.'));
              } else {
                $branch->delete();
              }
