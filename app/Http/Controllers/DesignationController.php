...
class DesignationController extends Controller
{
    ...
    public function destroy(Designation $designation)
    {
        if(\Auth::user()->can('Delete Designation'))
        {
            if($designation->created_by == \Auth::user()->creatorId())
            {
              $emps1 = \App\Employee::where("reporting_id",$designation->id)->get();
              $emps = \App\Employee::where("designation_id",$designation->id)->get();
              if(count($emps1) || count($emps) ) {
                return redirect()->route('designation.index')->with('error', __('Designation is being used in app.'));
              } else {
                $designation->delete();
              }
