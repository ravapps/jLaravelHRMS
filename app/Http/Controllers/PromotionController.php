...
class PromotionController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('Create Promotion'))
        {
            ...
            $promotion->employee_id     = $request->emp_ids;
            $promotion->designation_id  = $request->designation_id;
            $promotion->designation_to_id  = $request->designation_to_id;
            $promotion->promotion_date  = date('Y-m-d', strtotime($request->promotion_date));
    ...
    {
        $designations1 = Designation::where('created_by', Auth::user()->creatorId())->get();
        $employees    = Employee::where('created_by', Auth::user()->creatorId())->get()->pluck('name', 'id');


        $get_employee = Employee::where('id', $promotion->employee_id)->first();
        ...
        if(\Auth::user()->can('Edit Promotion'))
        {
            ...
            {
                return view('promotion.edit', compact('promotion', 'employees', 'designations','designations1','get_employee','department'));
            }
            else
            {
                return response()->json(['error' => __('Permission denied.')], 401);
